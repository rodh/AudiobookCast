...
function loadTags(dataDir) {
  try {
    const raw = fs.readFileSync(path.join(dataDir, TAGS_FILE), 'utf8');
    return JSON.parse(raw);
  } catch {
    return {};
  }
...
function mergeTags(books, tags) {
  for (const book of Object.values(books)) {
    const entry = tags[book.id];
...
module.exports = { loadCache, saveCache, mergeMetadata, loadTags, mergeTags };
